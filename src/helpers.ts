...
import OpenZeppelinTransparentProxyZk from '../extendedArtifacts/zk/TransparentUpgradeableProxy.json';
import OptimizedTransparentUpgradeableProxyZk from '../extendedArtifacts/zk/OptimizedTransparentUpgradeableProxy.json';
import DefaultProxyAdminZk from '../extendedArtifacts/zk/ProxyAdmin.json';
import eip173ProxyZk from '../extendedArtifacts/zk/EIP173Proxy.json';
import eip173ProxyWithReceiveZk from '../extendedArtifacts/zk/EIP173ProxyWithReceive.json';
import erc1967ProxyZk from '../extendedArtifacts/zk/ERC1967Proxy.json';
import diamondBaseZk from '../extendedArtifacts/zk/Diamond.json';
import diamondERC165InitZk from '../extendedArtifacts/zk/DiamondERC165Init.json';
import diamondCutFacetZk from '../extendedArtifacts/zk/DiamondCutFacet.json';
import diamondLoupeFacetZk from '../extendedArtifacts/zk/DiamondLoupeFacet.json';
import ownershipFacetZk from '../extendedArtifacts/zk/OwnershipFacet.json';
...
export function addHelpers(
  ...
  async function _getProxyInfo(
    ...
    let upgradeIndex;
    let proxyContract: ExtendedArtifact = network.zksync ? eip173ProxyZk : eip173Proxy;
        ...
        if (typeof options.proxy.proxyContract === 'string') {
          ...
          if (!proxyContract || proxyContract === eip173Proxy) {
            if (options.proxy.proxyContract === 'EIP173ProxyWithReceive') {
              ...
              proxyContract = network.zksync ? OpenZeppelinTransparentProxyZk : OpenZeppelinTransparentProxy;
              viaAdminContract = 'DefaultProxyAdmin';
            } else if (
              options.proxy.proxyContract === 'OptimizedTransparentProxy'
            ) {
              checkABIConflict = false;
              proxyContract = network.zksync ? OptimizedTransparentUpgradeableProxyZk : OptimizedTransparentUpgradeableProxy;
              viaAdminContract = 'DefaultProxyAdmin';
              ...
              proxyContract = network.zksync ? erc1967ProxyZk : erc1967Proxy;
...
Note that in this case, the contract deployment will not behave the same if deployed without proxy.
      ...
      if (typeof proxyAdminArtifactNameOrContract === 'string') {
        ...
        if (!proxyAdminContract) {
          if (viaAdminContract === 'DefaultProxyAdmin') {
            proxyAdminContract = network.zksync ? DefaultProxyAdminZk : DefaultProxyAdmin;
          } else {
            throw new Error(
  ...
  async function _deployViaDiamondProxy(
    ...
    let diamondArtifact: ExtendedArtifact = network.zksync ? diamondBaseZk : diamondBase;
    ...
    if (options.defaultCutFacet === undefined || options.defaultCutFacet) {
      facetsSet.push({
        ...
        contract: network.zksync ? diamondCutFacetZk : diamondCutFacet,
        args: [],
        deterministic: true,
      });
    }
    if (
      ...
      facetsSet.push({
        ...
        contract: network.zksync ? ownershipFacetZk : ownershipFacet,
        args: [],
        deterministic: true,
      });
    }
    facetsSet.push({
      ...
      contract: network.zksync ? diamondLoupeFacetZk : diamondLoupeFacet,
      args: [],
      deterministic: true,
    });
        ...
        if (initializationsArgIndex >= 0 || erc165InitArgIndex >= 0) {
          const diamondERC165InitDeployment = await _deployOne(
            '_DefaultDiamondERC165Init',
            {
              from: options.from,
              deterministicDeployment: true,
              contract: network.zksync ? diamondERC165InitZk : diamondERC165Init,
              autoMine: options.autoMine,
              estimateGasExtra: options.estimateGasExtra,
              estimatedGasLimit: options.estimatedGasLimit,
              gasPrice: options.gasPrice,
